<?php

namespace App\Policies;

use App\Slider;
use App\User;
use Illuminate\Auth\Access\HandlesAuthorization;
use Illuminate\Support\Facades\Gate;

class SliderPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the user can view any models.
     *
     * @param  \App\User  $user
     * @return mixed
     */
    public function viewAny(User $user)
    {
        return Gate::any(['viewSlider'], $user);
    }

    /**
     * Determine whether the user can view the model.
     *
     * @param  \App\User  $user
     * @param  \App\Slider  $category
     * @return mixed
     */
    public function view(User $user, Slider $slider)
    {
        return Gate::any(['viewSlider', 'manageSlider'], $user, $slider);
    }

    /**
     * Determine whether the user can create models.
     *
     * @param  \App\User  $user
     * @return mixed
     */
    public function create(User $user)
    {
        return Gate::any(['manageSlider'], $user);
    }

    /**
     * Determine whether the user can update the model.
     *
     * @param  \App\User  $user
     * @param  \App\Slider  $category
     * @return mixed
     */
    public function update(User $user, Slider $slider)
    {
        return Gate::any(['manageSlider'], $user, $slider);
    }

    /**
     * Determine whether the user can delete the model.
     *
     * @param  \App\User  $user
     * @param  \App\Slider  $category
     * @return mixed
     */
    public function delete(User $user, Slider $slider)
    {
        return Gate::any(['manageSlider'], $user, $slider);
    }

    /**
     * Determine whether the user can restore the model.
     *
     * @param  \App\User  $user
     * @param  \App\Category  $category
     * @return mixed
     */
    public function restore(User $user, Slider $slider)
    {
        return Gate::any(['manageSlider'], $user, $slider);
    }

    /**
     * Determine whether the user can permanently delete the model.
     *
     * @param  \App\User  $user
     * @param  \App\Slider  $category
     * @return mixed
     */
    public function forceDelete(User $user, Slider $slider)
    {
        return Gate::any(['manageCategory'], $user, $slider);
    }
}
